#To Do Tomorrow
#- Finish color design
#- implement logic to check if the users guess is a valid word- by seeing if it has a definition.
...
class Logic:
    ...
    def reset_game(self):
        self.dictionary_key = "065c2189-e6a7-47e8-8d2e-67f0733b1b35"
        ...
        self.wordle_characters = list(self.wordle)


        self.display.setup_gui()
    ...
    def get_target_word(self):
        while True:
            try:
                ...
                print("random word dictionary didnt work")

            word = (word_json[0]).upper()
            print(word)
    ...
    def get_definition(self, word):
        ...
        try:
            ...
            print("somethign went wrong with dictionary")
    ...
    def process_guess(self):
        ...
        # immediately checks if guess matches Wordle
...
class Display:
    ...
    def reveal_letter(self, index, letter, status): #letter variable might not be needed here
        self.canvas, self.text_item = self.row_mapping[self.logic.attempt][index]
        if status == "GREEN":
    ...
    def you_won(self, status):
        ...
        self.word_reveal = Label(self.congrats_window, text= 'the word was...',bg=color,
                           font=("Raleway", 20))

        self.word_reveal.grid(row=1, column=1, columnspan=3)

        self.wordle = Label(self.congrats_window, text= f'{self.logic.wordle}',bg=color, fg='purple',
                           font=("Raleway", 60))
        self.wordle.grid(row=2, column=1, columnspan=3)

        self.definition = Label(self.congrats_window, bg=color,text=f'Definition: {self.logic.definition}', wraplength=500,
                           font=("Raleway", 20))
        self.definition.grid(row=3, column=1, columnspan=3,pady=30)


        self.play_again = Button(self.congrats_window, text="PLAY AGAIN", height=3, width=10, font=("Raleway", 20),
                            command=self.logic.reset_game)
    ...
    def setup_gui(self):
        ...
        for column in range(2, 7):
            for row, row_list in enumerate([self.first_row, self.second_row, self.third_row, self.fourth_row, self.fifth_row, self.sixth_row],
                                           start=2):
                self.canvas = Canvas(root, width=square_size, height=square_size, bg="black", highlightthickness=1,
                                highlightbackground="grey")
                self.canvas.grid(row=row, column=column, padx=3, pady=3, columnspan=2)
                self.text_item = self.canvas.create_text(square_size / 2, square_size / 2, text="",
                                               font=("Helvetica", 20, "bold"), fill="white", anchor="center")
